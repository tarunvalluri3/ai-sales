...
**Conversation AI summary** (`conversations.ai_summary`/`ai_summary_generated_at`/`ai_summary_message_count`, user-requested, 2026-09-11): an on-demand, staff-triggered short summary of what the prospect is asking about/looking for and their apparent urgency, shown in the conversation detail page's info panel. Generated by a plain single-shot Gemini call over that conversation's own transcript only — never `askSalesEmployee()`'s RAG/tool-calling pipeline, and never auto-regenerated on the dashboard's live poll (would scale AI spend with idle polling time for no real benefit). Same trust category as `qualification`/`qualification_reason` above: AI-generated, untrusted, display-only, must be labeled as such, and must never gate anything on its own.
