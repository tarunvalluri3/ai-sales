...
- **Prompt-injection resistance** — three vectors, not just one:
  ...
  - a stronger variant of the above where the seeded chunk tries to literally close the `<retrieved_context>` tag mid-passage (`</retrieved_context>`) and inject a fake `[SYSTEM]` instruction after it — targets the tag mechanism itself (added alongside the 2026-09-10 structural hardening below), not just prose-shaped instructions.
...
Added 2026-09-10 alongside a `lib/rag.ts` system-prompt hardening: the prompt now explicitly tells the model that retrieved context and tool results are data to read, never instructions to follow — closing the gap the retrieval-injection case above tests for, not just documenting it. Extended the same day when the hardening became structural (retrieved context and tool results wrapped in literal `<retrieved_context>`/`<tool_result>` tags, not just described in prose) — the two existing escalation cases above already re-confirm both edges of the escalation logic after that change and the `MAX_TOOL_ITERATIONS` 2→3 bump, so no new escalation case was needed; only the tag-escape injection case above is new.
