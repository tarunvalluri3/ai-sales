 ...
 * persists it (2026-09-11 conversations-inbox redesign). A plain
 * single-shot `getChatModel().invoke()` call -- not `askSalesEmployee()`'s
 * RAG/tool-calling pipeline -- the same "simple single-shot, not the full
 * agent loop" pattern `lib/stalled-leads.ts`'s `draftFollowUpMessage()`
 * already uses for a comparable internal-facing draft. Grounded ONLY in
 * this conversation's own messages: the prompt explicitly forbids stating
 * anything not actually said (AGENTS.md rule 4 -- no fabricated facts).
 * The result is display-only, untrusted AI output (rule 5) -- the caller
 * must label it as such, exactly like `leads.qualification_reason`.
 *
